...
int main(int argc, char **argv) {
	...
	for (size_t input_idx = 0; input_idx < input_paths.size(); ++input_idx) {
		...
		if (file->GetListOfKeys()->Contains("ProcessedWaveforms")) {
			...
			cerr << "ERROR: Input file does not contain recognized tree (WCTEReadoutWindows or ProcessedWaveforms)" << endl;
			return -1;
		}
		tree = file->Get<TTree>(tree_name.c_str());
		...
		if (hardware_processed_data) {
			arr_pmt_times = new vector<double>();
			arr_pmt_ids   = new vector<int>();
			arr_mpmt_ids  = new vector<int>();
		}
		...
		double processed_hit_times[MAX_HITS];
		int processed_hit_cards[MAX_HITS];
		int processed_hit_chans[MAX_HITS];
		...
		if (hardware_processed_data) {
			tree-> SetBranchStatus("hit_time", 1);
			tree-> SetBranchStatus("hit_card", 1);
			tree-> SetBranchStatus("hit_chan", 1);
			tree-> SetBranchStatus("nhit_time", 1);
			tree-> SetBranchStatus("nhit_card", 1);
			tree-> SetBranchStatus("nhit_chan", 1);
			tree->SetBranchAddress("hit_time", processed_hit_times);
			tree->SetBranchAddress("hit_card", processed_hit_cards);
			tree->SetBranchAddress("hit_chan", processed_hit_chans);
			tree->SetBranchAddress("nhit_time", &n_pmt_time);
			tree->SetBranchAddress("nhit_card", &n_pmt_card);
			tree->SetBranchAddress("nhit_chan", &n_pmt_chan);
		} else {
		...
		for(long long i = 0; i < n_events; i++){
			...
			if (hardware_processed_data) {
				if (n_pmt_time>=MAX_HITS){
					//continue and push back dummy detections
					std::cerr << "ERROR: Too many hits in event " << i <<" nhits "<<  MAX_HITS<< " (max " << MAX_HITS << ")" << std::endl;
					detections.event_nr = i;
					all_T5_hits.push_back(detections);
					continue;
				}
				//change array output into std vectors
				arr_pmt_times->assign(processed_hit_times, processed_hit_times + n_pmt_time);
				arr_mpmt_ids->assign(processed_hit_cards, processed_hit_cards + n_pmt_card);
				arr_pmt_ids->assign(processed_hit_chans, processed_hit_chans + n_pmt_chan);
			}
		...
		if (!output_file || output_file->IsZombie()){
			cerr << "ERROR: Did not open output file: " << current_output_path << endl;
			continue;
		}
